...
<div class="container py-3">
    <div class="container-fluid d-flex justify-content-between">
        <h4 class="card-title">Data Permintaan ATK</h4>
            <a href="{{ route('atk.create') }}" class="btn btn-primary mb-3">Tambah Permintaan</a>
    </div>
    @if(session('success'))
          ...
          class="display table table-striped table-hover">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($item->status === 'Disetujui')
                            <a href="{{ route('atk.print', $item->id) }}" class="btn btn-info btn-sm">
                                </i> Cetak
                            </a>
                        @endif
                        @if($item->status === 'Pengajuan')
                            <a href="{{ route('atk.edit', $item->id) }}" class="btn btn-warning btn-sm">Edit</a>
                            <form action="{{ route('atk.destroy', $item->id) }}" method="POST" style="display:inline-block;">
                                @csrf
                                @method('DELETE')
