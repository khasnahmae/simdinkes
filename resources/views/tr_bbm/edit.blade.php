    ...
    <div class="container py-3">
        ...
        <form action="{{ route('tr_bbm.update', $bbm->uuid) }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <select name="nopol" class="form-control" id="nopol" required>
                                ...
                                @foreach ($kendaraan as $kn)
                                    <option value="{{ $kn->id }}" {{ $kn->id == $bbm->nopol ? 'selected' : '' }}>
                                        {{ $kn->nopol }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" name="nama_kendaraan" class="form-control" id="nama_kendaraan"
                                value="{{ $bbm->nama_kendaraan }}" readonly required>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <label for="jenis_bbm">Jenis BBM</label>
                            <input type="text" name="jenis_bbm" class="form-control" id="jenis_bbm"
                                value="{{ $bbm->jenis_bbm }}" readonly required>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <label for="nominal">Nominal</label>
                            <input type="number" name="nominal" class="form-control" id="nominal" step="0.01"
                                value="{{ $bbm->nominal }}" required>
                        </div>
                    </div>
                </div>
            </div>
            <div class="form-group justify-content-start">
                ...
                <a href="{{ route('tr_bbm.index') }}" class="btn btn-outline-secondary btn-sm">Kembali</a>
            </div>
        </form>
    </div>
